...
def post_to_ldn_inbox(events: iter=None,
    ...
    for event in events:
        ...
        # preventing duplicate events to be sent from within
        # a single tracking event for a user+portal
        items = event.get("event", {}).get("object", {}).get("items", [])
        if len(items) == 0:
            continue
        href = items[0].get("href")
        pkey = href + event.get("event", {}).get("published")
        if seen.get(pkey):
            tracker_app.log.debug("Duplicate event found in the event list from tracker. skipping...")
            continue
        seen[pkey] = None
        ...
        inbox_message.message = json.dumps(event).encode("utf-8")
        inbox_message.message_type = "out"
        inbox_message.created_at = datetime.now()
        tracker_app.db.session.add(inbox_message)
