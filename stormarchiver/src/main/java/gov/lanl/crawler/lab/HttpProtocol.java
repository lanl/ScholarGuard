...
public class HttpProtocol extends AbstractHttpProtocol {
    ...
    public ProtocolResponse getProtocolOutput(String url, final Metadata metadata) throws Exception {
        ...
        try (Response response = call.execute()) {
            ...
            for (int i = 0, size = headers.size(); i < size; i++) {
                ...
                if (key.equals(REQUEST_HEADERS_KEY)
                        || key.equals(RESPONSE_HEADERS_KEY)) {
            ...
            if (trimmed.getValue() != TrimmedContentReason.NOT_TRIMMED) {
                ...
                responsemetadata.setValue(TRIMMED_RESPONSE_KEY,
                        "true");
            ...
            StringBuilder dummyContent = new StringBuilder("<html></html>");
            
            
            //return new ProtocolResponse(bytes, response.code(), responsemetadata);
            return new ProtocolResponse(dummyContent.toString().getBytes(), response.code(), responsemetadata);
        }
    }
    ...
    class HTTPHeadersInterceptor implements Interceptor {
        ...
        public Response intercept(Interceptor.Chain chain) throws IOException {
            ...
            return response
                    .newBuilder()
                    .header(REQUEST_HEADERS_KEY,
                            new String(encodedBytesRequest))
                    .header(RESPONSE_HEADERS_KEY,
                            new String(encodedBytesResponse))
                    .header(RESPONSE_IP_KEY, ipAddress)
                    .header(REQUEST_TIME_KEY,
